...
import { X, LogOut, Rss, User, Coffee } from 'lucide-react';
...
export default function Sidebar({ open, onClose }) {
  ...
  return (
    <aside
      className={`fixed top-0 left-0 z-50 flex h-full w-64 flex-col border-r border-neutral-200 bg-white transition-transform duration-200 lg:translate-x-0 ${
    ...
    >
      ...
      <div className="flex h-14 items-center justify-between border-b border-neutral-200 px-5">
        ...
        <button
          onClick={onClose}
          className="rounded-lg p-1 text-neutral-400 hover:bg-neutral-100 hover:text-neutral-600 lg:hidden"
        >
          <X size={18} />
        </button>
      </div>
      ...
      <div className="border-t border-neutral-200 px-3 py-3">
        {profile && (
          <p className="mb-2 truncate px-3 text-xs text-neutral-400">
            {profile.name} &middot; {profile.year_level}
          </p>
        )}
        <button
        ...
        <a
          href="#"
          className="mt-2 flex w-full items-center gap-2 rounded-lg px-3 py-2 text-xs text-neutral-400 transition-colors hover:text-neutral-600"
        >
          <Coffee size={13} />
          Buy me a coffee
        </a>
